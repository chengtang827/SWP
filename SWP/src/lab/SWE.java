...
public class SWE
{
	...
	public void init()
	{
		try
		{
			...
			FrameHandler localFrameHandler = new FrameHandler(this.mydis, this);//used to wait and receive frames
			localFrameHandler.start();

			NetworkSender localNetworkSender = new NetworkSender(this, this.siteid);
			localNetworkSender.start();//extract from file and send if have space
	...
	public synchronized void get_credit()
	{
		...
		////////////////////////////////////////////
		while (this.credit <= 0) try {
			super.wait(); } catch (InterruptedException localInterruptedException) {
			}
		this.credit -= 1;
	}
